...
@section('panel')
    <div class="row">
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.catalog.product.store') }}" method="POST" class="disableSubmission" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-xl-4">
                                <div class="form-group">
                                    <label>@lang('Product Image')</label>
                                    <x-image-uploader class="w-100" :imagePath="frontendImage('',@$data->image,getFileSize('product'),true)" :size="getFileSize('product')" :required="false" />
                                </div>
                            </div>
                            ...
                            <div class="col-xl-8 mt-xl-0 mt-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="text" rows="3" class="form-control">{{ old('text') }}</textarea>
                                </div>
                                <div class="form-group">
                                        @if (@$properties)
                                            @foreach ($properties as $property)
                                            <label>{{$property->title}}</label>
                                            <input type="text" class="form-control" name="properties[{{$property->id}}]" value="@php old('properties['.$property->id.']') @endphp">
                                            @endforeach
                                        @endif
                                </div>
                                <div class="form-group">
