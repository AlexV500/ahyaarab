...
class AdminController extends Controller {
    ...
    public function otherProfile($id) {
        $admin     = auth('admin')->user();
        $otherAdmin      = Admin::findOrFail($id);
        $pageTitle = 'Admin Profile - ' . $otherAdmin->username;
        return view('admin.otherprofile', compact('pageTitle', 'otherAdmin'));
    }

    public function otherProfileUpdate(Request $request) {

        $otherAdmin = Admin::where('id', $request->id)->firstOrFail();
        ...
        if ($request->hasFile('image')) {
            try {
                $old         = $otherAdmin->image;
                $otherAdmin->image = fileUploader($request->image, getFilePath('adminProfile'), getFileSize('adminProfile'), $old);
            } catch (\Exception $exp) {
        ...
        $otherAdmin->name  = $request->name;
        $otherAdmin->email = $request->email;
        $otherAdmin->superadmin = $request->superadmin;
        $otherAdmin->save();
        $notify[] = ['success', 'Profile updated successfully'];
        return to_route('admin.profiles')->withNotify($notify);
    }
    public function profiles() {
        $pageTitle = 'Admin Profiles';
        //    $admin     = auth('admin')->user();
        $administrators = Admin::orderBy('id', 'desc')->paginate(getPaginate());
        return view('admin.profiles', compact('pageTitle', 'administrators'));
    }

    public function addprofile() {
        $pageTitle = 'Add Admin Profile';
        return view('admin.addprofile', compact('pageTitle'));
    }
